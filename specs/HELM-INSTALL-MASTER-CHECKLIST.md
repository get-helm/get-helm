...
| C24 | **Connector setup prompt** | Asks which email/calendar/drive **provider** (never assumes Gmail), then OAuths it in-flow | ✅ **DECIDED — deliver basics in-flow** ({{USER_JERRY}} 2026-06-15). Briefing must work before user exits onboarding. | Engineer: build provider-ask + basic OAuth |
...
| Connector promise in onboarding | Remove vs. defer vs. deliver | ✅ **DECIDED — deliver: offer to set up the basics in-flow** ({{USER_JERRY}} 2026-06-15). Daily briefing must work before user exits onboarding. | {{USER_JERRY}} |
