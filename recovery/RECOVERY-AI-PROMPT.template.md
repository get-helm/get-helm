...
**What HELM is:**
HELM is a Discord bot that routes my messages to AI agents. It runs on a dedicated computer at home (called the "clean machine" — either a Mac Mini or Windows PC). My Discord server ID is {{DISCORD_SERVER_ID}}.

**My setup:**
- Clean machine type: {{MACHINE_TYPE}}
- VPS (remote server): {{VPS_ENABLED}}
- Bot name: {{BOT_NAME}}
...
- If none of the above helps: the issue may require hands-on access. Post in your HELM questions channel — even a partially-working HELM monitors it. If HELM is fully down, restart the clean machine and wait 5 minutes before trying again.{{#SUPPORT_CONTACT}} Last resort: contact {{SUPPORT_CONTACT}}.{{/SUPPORT_CONTACT}}
